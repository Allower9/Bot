...
def test_create_app_with_mock_application():
    """Тест создания приложения с моком Application"""
    with patch("bot.main.Application") as MockApplication:
        mock_app = MagicMock()
        MockApplication.builder.return_value.token.return_value.build.return_value = (
            mock_app
        )
